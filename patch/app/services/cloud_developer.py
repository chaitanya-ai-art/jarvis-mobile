...
class DeveloperJobs:
    ...
    def create(self, instruction: str) -> DeveloperJob:
        job = DeveloperJob(id=uuid.uuid4().hex, instruction=instruction.strip())
        with self._lock:
    ...
    def run(self, job_id: str) -> None:
        ...
        try:
            self._update(job, status="working", message="Reading GitHub project")
            ...
            branch = f"jarvis-ai/{datetime.now(timezone.utc).strftime('%Y%m%d-%H%M%S')}"
            api.create_branch(branch, main_sha)
            self._update(job, branch=branch, message="Analyzing source code")
            ...
            changes = generate_changes(job.instruction, files, openai_key)
            if not changes:
            ...
            self._update(
                ...
                status="ready",
                message=(
                    "Development branch created. Review it on GitHub before merging."
                ),
...
class GitHubRepo:
    ...
    def branch_sha(self, branch: str) -> str:
        data = self._request("GET", f"{self.base}/git/ref/heads/{branch}")
    ...
    def get_file(self, path: str, branch: str) -> tuple[str, str] | None:
...
def validate_path(path: str) -> str:
    ...
    if clean == ".env" or any(clean.startswith(p) for p in protected[1:]):
        raise RuntimeError(f"Protected path: {clean}")
    if not re.search(r"\.(py|html|css|js|json|md|txt|yml|yaml|bat)$", clean):
...
- Mobile-first; laptop controls must remain optional.
- Never expose or edit secrets.
- Never weaken authentication.
- Never add arbitrary shell execution, surveillance, credential collection,
  destructive actions, or automatic merging/deployment.
